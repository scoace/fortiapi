...
class FortiGate:
    ...
    def get_routes(self):
        ...
        api_url = self.urlbase + "api/v2/monitor/router/ipv4"
        results = self.get(api_url)
        return results

    

    def add_firewall_address(self, name, ip_mask):

        payload = {'name': name, 'type': 'subnet', 'subnet': ip_mask}
        rc = self.create_firewall_address(name, repr(payload))
    ...
    def add_firewall_group(self, name,list_of_members):
        ...
        for member in list_of_members:
            tmpdict['name']=member
            glist.append(tmpdict)   
            tmpdict={}
        payload = {'name': name , 'member': glist}
        rc=self.create_address_group(name,repr(payload))
        
        return rc

    def add_route(self, list_of_attributes): 
        # [device,dst,gateway,comment]
        if (len(list_of_attributes))!=4:
            print ("Wrong count of arguments")
            return(1)    
        data={"device": list_of_attributes[0],"dst": list_of_attributes[1],"gateway": list_of_attributes[2],"comment": list_of_attributes[3]}
        api_url = self.urlbase + "api/v2/cmdb/router/static"
        
        rc=self.post( api_url,repr(data))
        
        return(rc)

    def get_interface(self,specific=False,filters=False):
    ...
    def update_system_interface(self,interface_name,dict_of_attributes):
        """
        Update System Interface
        :param interface name, dict of attributes
        :200 if OK
        """
        api_url = self.urlbase + "api/v2/cmdb/system/interface/"+interface_name
        if not self.does_exist(api_url):
